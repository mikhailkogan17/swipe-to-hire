...
// Used by buildProfileSetupGraph to wait for user's answer to a clarification question.
// Wired in bot.on('text') handler below and in api.ts /onboarding/complete.
...
bot.command('start', async ctx => {
  ...
  if (!user.onboarded) {
    ...
    await ctx.reply('👋 Welcome back\\! Your job search is running\\.', {
      parse_mode: 'MarkdownV2',
      ...Markup.inlineKeyboard([
...
bot.on('document', async ctx => {
  ...
  if (pendingStates.get(telegramUserId) === 'awaiting_cv') {
    ...
    const rapidApiKey = user.rapidapi_key ?? env.RAPIDAPI_KEY;
    ...
    import('@swipe-to-hire/agent/graph.js')
      ...
      .catch(async err => {
        ...
        await bot.telegram.sendMessage(
          telegramUserId,
          '❌ Profile extraction failed. Please try again or contact support.'
        );
      });
...
bot.on('text', async ctx => {
  ...
  // --- Profile-setup HITL: route reply to waiting graph node ---
  ...
  switch (intent.intent) {
    ...
    case 'update_prefs': {
      if (!intent.preferencesDelta || Object.keys(intent.preferencesDelta).length === 0) {
        await ctx.reply(
          "I understood you want to update preferences, but couldn't extract what to change. Could you be more specific?"
        );
        break;
      }
...
export async function notifyJobsReady(telegramUserId: number, count: number): Promise<void> {
  await bot.telegram.sendMessage(
    ...
    {
      parse_mode: 'MarkdownV2',
      ...Markup.inlineKeyboard([Markup.button.webApp('👆 Review Jobs', env.WEBAPP_URL)]),
    }
  );
}
